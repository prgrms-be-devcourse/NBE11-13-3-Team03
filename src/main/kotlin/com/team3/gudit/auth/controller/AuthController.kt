...
""",
    ...
    @PostMapping("/logout")
    fun logout(@AuthenticationPrincipal user: CustomUserDetails, response: HttpServletResponse): ResponseEntity<Void> {
...
""",
    ...
    fun reissue(
        ...
        val tokens = tokenService.reissueToken(refreshToken)
        addTokenCookie(response, "access_token", tokens.accessToken, jwtProperties.accessTokenValidity.seconds.toInt())
        addTokenCookie(response, "refresh_token", tokens.refreshToken, jwtProperties.refreshTokenValidity.seconds.toInt())
        return ResponseEntity.ok().build()
    }

    private fun addTokenCookie(response: HttpServletResponse, name: String, value: String, maxAge: Int) {
